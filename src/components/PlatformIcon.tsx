...
// Proper LinkedIn icon via Simple Icons (official path) — react-icons/si lacks it in this version (v5.7)
// Using exact Simple Icons path so rendering is pixel-perfect and not dependent on Fa
function LinkedinIcon({ className, style }: { className?: string; style?: React.CSSProperties }) {
  // Extract size from style for explicit width/height attributes (ensures SVG sizes correctly even with style overrides)
  const sizeW = (style as unknown as { width?: number | string })?.width as number | undefined;
  const sizeH = (style as unknown as { height?: number | string })?.height as number | undefined;
  return (
    <svg
      role="img"
      viewBox="0 0 24 24"
      className={className}
      style={style}
      width={sizeW as unknown as string | number}
      height={sizeH as unknown as string | number}
      fill="currentColor"
      aria-hidden="true"
